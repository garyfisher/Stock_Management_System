...
class ProductsModel extends AbsModel
{
    ...
    public $ProductId, $kod, $Title, $Tax, $UnitId , $MadeCountry, $AddedDate, $CategoryId, $Quantity, $NotificationQuantity, $Barcode, $SellPrice, $BuyPrice, $PromoPrice;
    ...
    {
        $insetValues = [$this->kod,$this->Title,$this->MadeCountry,$this->Tax,$this->UnitId,$this->CategoryId,$this->Quantity,$this->NotificationQuantity,$this->Barcode,$this->SellPrice,$this->BuyPrice,$this->PromoPrice];
        return DB::insert('insert into '. self::TABLE .' (kod,Title,MadeCountry,Tax,UnitId,CategoryId,Quantity,NotificationQuantity,Barcode,SellPrice,BuyPrice,PromoPrice,AddedDate) values (?,?,?,?,?,?,?,?,?,?,?,now())',$insetValues);
    }

    public function update()
    {
        $updateValues = [$this->kod,$this->Title,$this->MadeCountry,$this->Tax,$this->UnitId,$this->CategoryId,$this->Quantity,$this->NotificationQuantity,$this->Barcode,$this->SellPrice,$this->BuyPrice,$this->PromoPrice,$this->ProductId];
        return DB::update("update ". self::TABLE ." set  kod=?, Title=?, MadeCountry=?, Tax=?,UnitId=? , CategoryId=? , Quantity=?, NotificationQuantity=?, Barcode=?, SellPrice=?, BuyPrice=?, PromoPrice=? WHERE ProductId=?",$updateValues);
    }
